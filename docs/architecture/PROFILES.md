...
Bu doküman, öğrencilerin eğitim boyunca kullanacağı **2 vCPU, 16 GB RAM ve 50 GB disk** kapasiteli sanal sunucu (VM) ortamında sistemin stabil çalışmasını sağlamak amacıyla tanımlanmış **çalıştırma profillerini (execution profiles)** ve kaynak yönetim kurallarını açıklar (Karar: `D-008`).
...
| Profil Adı | İlgili Milestone / Lab | Dahil Olan Servisler | Hedef CPU | Hedef RAM | Disk Tüketimi |
|---|---|---|---|---|---|
| **`starter`** | M01, M03 (Starter) | NovaShop UI (In-Memory Mock) | 0.5 vCPU | 512 MB | ~300 MB |
| **`compose-core`** | M03, M04 | UI, Catalog, MySQL (Catalog DB) | 1.0 vCPU | 1.5 GB | ~1.5 GB |
| **`compose-full`** | M03 (Gelişmiş) | UI, Catalog, Cart, Checkout, Orders, MySQL, DynamoDB-local, Redis, Postgres, RabbitMQ | 1.8 vCPU | 4.0 GB | ~4.5 GB |
| **`cicd-github`** | M05 | Runner / Local Action Tools, ECR Proxy | 1.0 vCPU | 2.0 GB | ~3.0 GB |
| **`cicd-enterprise`** | M07 | GitLab CE veya Jenkins + Harbor Registry | 1.8 vCPU | 8.0 GB | ~12.0 GB |
| **`security-gates`** | M08 | SonarQube Server + PostgreSQL + Trivy | 1.5 vCPU | 4.0 GB | ~5.0 GB |
| **`k8s-core`** | M06 | Kind Cluster (1 Control-Plane, 2 Worker) + NovaShop Core Helm | 1.8 vCPU | 6.0 GB | ~8.0 GB |
| **`gitops-argo`** | M09 | Kind Cluster + Argo CD Controller/Server | 1.8 vCPU | 6.5 GB | ~8.5 GB |
| **`observability`** | M10 | Prometheus, Grafana, Alertmanager, Jaeger, OTel Collector | 1.5 vCPU | 4.0 GB | ~6.0 GB |
| **`logging-elk`** | M11 | Fluent Bit, Elasticsearch (Tek Düğüm), Kibana | 1.8 vCPU | 6.0 GB | ~10.0 GB |
